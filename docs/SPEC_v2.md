...
完成バンクは練習開始時に遅延取得し、Stage Schema v2で検証する。取得・検証を8秒以内に完了できない場合は、既存30問の`legacy-v1`へ自動fallbackする。一時的なfallback結果は固定せず、次の練習開始時に再取得する。完成バンクの取得成功とfeature gate無効時のfallbackだけは同じページ内で再利用する。
